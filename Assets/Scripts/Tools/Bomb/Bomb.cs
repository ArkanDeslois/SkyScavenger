...
public class Bomb : MonoBehaviour
{
    ...
    void Bomb_Activation()
    {
        explo = false;
        active_Bomb = true;
        player_bomb_instance_coordinates = new Vector3(this.transform.position.x, this.transform.position.y, this.transform.position.z + 1.2f);
        Instantiate(prefab_Bomb, player_bomb_instance_coordinates, Quaternion.identity);
        bombPrefab_Placeholder = GameObject.Find("Bomb_Prefab(Clone)");
    }
